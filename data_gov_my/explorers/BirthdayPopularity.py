...
class BIRTHDAY_POPULARITY(General_Explorer):
    ...
    def  handle_api(self, request_params):
       ...
       # handle the data 
        timeseries = DashboardJson.objects.get(dashboard_name="birthday_popularity", chart_name="timeseries").chart_data
        rank_table = DashboardJson.objects.get(dashboard_name="birthday_popularity", chart_name="rank_table").chart_data
        ...
        start = int(request_params["start"][0]) if "start" in request_params else (datetime.utcfromtimestamp(0) + timedelta(milliseconds=epochs[0])).year
        end = int(request_params["end"][0]) if "end" in request_params else (datetime.utcfromtimestamp(0) + timedelta(milliseconds=epochs[-1])).year
        groupByDay = self.str2bool(request_params["groupByDay"][0]) if "groupByDay" in request_params else True
        birthday = datetime.strptime(request_params["birthday"][0], "%Y-%m-%d") if "birthday" in request_params else None
        ...
        for i, e in enumerate(epochs):
            ...
            if date == birthday:
                res["rank"] = ranks[i]
                res["state_total"] = births[i]
                res["nationwide_total"] = nationwide_births[i]
            if start <= date <= end:
        ...
        if groupByDay:
            valid_dates = self.dates_in_year(1972) if hasLeap else self.dates_in_year(1970)
        else:
        ...
        if birthday is not None:
             res["popularity"] = rank_table["data"][state][str(birthday.year)]
