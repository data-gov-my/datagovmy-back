...
from data_gov_my.serializers import (
    ...
    ElectionSeatSerializer,
    ElectionPartySerializer,
    ElectionOverallSeatSerializer,
)
...
class ELECTIONS(General_Explorer):
    ...
    def handle_api(self, request_params):
        # Handles Dropdowns
        if (
            "dropdown" in request_params
            and request_params["dropdown"][0] in self.dropdowns
        ):
        ...
        if "chart" in request_params and request_params["chart"][0] in self.charts:
            ...
            if chart_type == "full_result":
                ...
                res = self.chart_handler(
                    request_params=request_params, chart=chart_type
                )
    ...
    def get_dropdown(self, dropdown_type=""):
        ...
        if dropdown_type == "seats_list":
            ...
            for x in list(model_choice.objects.values()):
                data[x.get("state")].append(
                    {"name": x["election"], "year": str(x["date"])}
                )
        else:
    ...
    def chart_handler(self, request_params, chart):
        ...
        filters = {
            k: request_params[v][0] for k, v in chart_choice["params_mapping"].items()
        }
        ...
        if chart_choice["segment_by"]:
            ser_data = self.group_by_type(
                type=chart_choice["segment_by"], data=ser_data
            )
    ...
    def overall_seat(self, request_params):
        ...
        r_data = {"data_last_updated": last_update, "data": serializer.data}
        res["msg"] = r_data
        res["status"] = 200
    ...
    def full_result(self, request_params):
        ...
        c_serializer = (
            ElectionCandidateSerializer
            if extract_votes_outcome
            else ElectionPartySerializer
        )
        election = request_params["election"][0]
        ...
        if extract_votes_outcome and len(data) > 0:
            ...
            r["data"] = [
                {k: v for k, v in d.items() if k not in const_keys} for d in data
            ]
            data = r

        last_update = self.get_last_update(model_name=model_name)

        r_data = {"data_last_updated": last_update, "data": data}
        res["msg"] = r_data
        res["status"] = 200
